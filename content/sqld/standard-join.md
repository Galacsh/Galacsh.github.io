...
- 일반 집합 연산자
  ...
  - INTERSECTION (교집합)
  - DIFFERENCE (차집합)
  - PRODUCT (곱집합 또는 Cartesian product)
- 순수 관계 연산자
  - SELECT
  - PROJECT
  - (NATURAL) JOIN
...
| 연산         | 기능                             | 설명                                                                                                                   |
| ------------ | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| UNION        | UNION                            | 수학적 합집합<br/>공통 교집합의 중복을 업애기 위한 사전 작업<br/>정렬 작업 발생                                        |
|              | UNION ALL                        | 공통 집합을 중복해서 그대로 보여줌<br/>정렬 작업이 발생하지 않음                                                       |
| INTERSECTION | INTERSECT                        | 수학의 교집합<br/>두 집합의 공통집합 추출                                                                              |
| DIFFERENCE   | EXCEPT (표준)<br/>MINUS (Oracle) | 수학의 차집합<br/>첫 번째 집합에서 두 번째 집합과의 공통집합을 제외한 부분                                             |
| PRODUCT      | CROSS JOIN<br/>(ANSI/ISO 표준)   | 곱집합 또는 Cartesian Product<br/>JOIN 조건이 없는 경우 생길 수 있는 모든 데이터의 조합<br/>M\*N 건의 데이터 조합 발생 |
...
| 연산           | 기능          | 설명                                                                                                        |
| -------------- | ------------- | ----------------------------------------------------------------------------------------------------------- |
| SELECT         | WHERE 절      | WHERE 절의 조건절 기능<br/>SELECT 연산과 SELECT 절은 다른 것임                                              |
| PROJECT        | SELECT 절     | SELECT 절의 컬럼 선택 기능                                                                                  |
| (NATURAL) JOIN | 다양한 JOIN   | WHERE 절의 INNER JOIN 조건<br/>FROM 절의 NATURAL, INNER, OUTER JOIN<br/>USING 조건절, ON 조건절 등으로 발전 |
| DIVIDE         | 사용되지 않음 | 나눗셈과 비슷한 개념                                                                                        |

---

# 조인

> 정규화를 거치면 하나의 주제에 관련 있는 엔터티가 여러 개로 나누어지게 흩어지고,
> 이렇게 흩어진 데이터를 연결해서 원하는 데이터를 가져오는 작업임
...
정규화 과정의 경우 데이터 정합성과 데이터 저장 공간의 절약을 위해 엔터티를 최대한 분리하는 작업이며
일반적으로 3차 정규형이나 보이스코드 정규형까지 진행한다.
...
- [표준 조인 - 데이터온에어](https://dataonair.or.kr/db-tech-reference/d-guide/sql/?pageid=3&mod=document&uid=346)
